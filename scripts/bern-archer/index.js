...
const S_SLOWTRAP = 90700; 
const S_SLOWTRAP_D =1130;

const S_STUNTRAP = 100400; 
const S_STUNTRAP_D =1145;

const S_VELIK = 120300; 
const S_VELIK_D = 200; 

const S_INCINTRAP = 150700; 
const S_INCINTRAP_D =1130;

const S_BREAKAWAY = 160600; 
...
const S_POISON = 190800; 
const S_POISON_D =1110;

const S_RESTRAIN = 200200; 
const S_RESTRAIN_D =510;

const S_SEQFIRE = 221100; 
const S_SEQFIRE_D =420;

const S_STUNRTRAP = 230200; 
...
const S_FINDWEAK = 320300; 
const S_FINDWEAK_D = 200; 
...
const ARROWRAINLOCK = 1100; 
const ARROWRAINLOCK2 = 4150; 
const S_SNIPEREYE =210200;

const S_DEATH =70300;

const Breeze_Steriod =350100;
const Breeze_Steriod_2 =350130;
...
module.exports = function archer(dispatch) {
	...
	let GUST_CANCEL_FIX =0;
    if (("GUST_CANCEL_FIX" in config)) {
        GUST_CANCEL_FIX = config.GUST_CANCEL_FIX;
    }
    if (!("GUST_CANCEL_FIX" in config)) {
        config.GUST_CANCEL_FIX =0;
        config.GUST_CANCEL_FIX_DESCRIPTION = "If your gust arrow is being cancelled by SEQ/WW macro too rapidly, try increasing this number. Go up in increments of 50.";
        settingUpdate();
    }
	...
	let WINDSONG_X = false;
	if (("WINDSONG_X" in config)) {
		WINDSONG_X = config.WINDSONG_X;
	}
	if (!("WINDSONG_X" in config)) {
		config.WINDSONG_X = false;
		config.WINDSONG_X_DESCRIPTION = "Auto use X_KEY when Wind Song is used. X_KEY must be set. This macro does not work without ROBOTJS.";
		settingUpdate();
	}
    ...
    dispatch.hook('C_CHAT', 1, (event) => {
		...
		if(event.message.includes("fd1")){
			FD_CANCEL = !FD_CANCEL;
			console.log("FD_CANCEL state: "+FD_CANCEL);
			return false;
		}
    });
    ...
    dispatch.hook('S_CREST_MESSAGE', 2, { order: -99999 }, (event) => {
        if (!enabled) { return };
        if (RF_RESET_STOP && lastSkill != Breeze_Steriod && lastSkill != Breeze_Steriod_2 && event.skill.id == (S_RAPIDFIRE)) {
            dispatch.toClient('S_START_COOLTIME_SKILL', 3, {
                skill: S_RAPIDFIRE,
                cooldown: (20 + 2340 / aspd),
            });
        }
        if (RF_RESET_STOP && lastSkill != Breeze_Steriod && lastSkill != Breeze_Steriod_2 && event.skill.id == (S_RAPIDFIRE_B)) {
            dispatch.toClient('S_START_COOLTIME_SKILL', 3, {
                skill: S_RAPIDFIRE_B,
                cooldown: (20 + 2340 / aspd),
            });
	...
	dispatch.hook('S_CANNOT_START_SKILL', 4, (event) => {
		...
		if(event.skill.id == S_ARROWVOLLEY){
			dispatch.toServer('C_START_SKILL', 7, lastvolley);
		}
	});
    ...
    dispatch.hook('C_START_INSTANCE_SKILL', 7, (event) => {
        ...
        if (!disabSkill[event.skill.id]) {
            ...
            if (job == JOB_ARCHER && event.skill.id == S_ARROW) {
                disabSkill[event.skill.id] = true;
                var timer = setTimeout(function () { disabSkill[S_ARROW] = false; }, GLOBAL_LOCK_DELAY / aspd);
    ...
    dispatch.hook('C_START_SKILL', 7, (event) => {
        ...
        if (!disabSkill[event.skill.id] && (finish[SKILL_CHARGING] != false || event.skill.id == S_BACKSTEP)) {
            ...
            if (xzzy && event.skill.id != S_ARROWVOLLEY && event.skill.id != S_RETAL) {
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} dispatch.toServer('C_START_SKILL', 7, event); } }, 25, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} dispatch.toServer('C_START_SKILL', 7, event); } }, 50, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} dispatch.toServer('C_START_SKILL', 7, event); } }, 75, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} dispatch.toServer('C_START_SKILL', 7, event); } }, 100, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} dispatch.toServer('C_START_SKILL', 7, event); } }, 150, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} dispatch.toServer('C_START_SKILL', 7, event); } }, 200, event);
            }
			lastarrow =0;
            ...
            if (job == JOB_ARCHER && (event.skill.id == Breeze_Steriod || event.skill.id == Breeze_Steriod_2)) {
				...
				var timer2 = setTimeout(function () { disabSkill[Breeze_Steriod_2] = false; }, GLOBAL_LOCK_DELAY);
            ...
            if (job == JOB_ARCHER && event.skill.id == (TEMPEST_TRASH_ARROW_R -1)) {
				...
				if (AUTOWW_GUST && (seqfiredisable || !AUTOSEQFIRE_GUST)) {
                    ...
                    setTimeout(function(event){
                    repeater(WWKEY, event.skill.id);
					}, 300 / aspd + GUST_CANCEL_FIX, event);
                }
            }
...
disabSkill[(S_ARROWRAIN +50)] = false;
    ...
    dispatch.hook('S_ACTION_END', 5, (event) => {
	  if(event.skill == 22202222) console.log("error");
        if (!enabled) return;
        if (event.gameId === cid) {
            var d = new Date();
