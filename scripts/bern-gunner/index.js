...
const S_AB_SPEC1 =71131;
const S_AB_SPEC =71132;
const S_AB_SPEC2 =71133;
...
module.exports = function gunner(dispatch) {
	...
	let ScatterABCancelState = false;
	if (("ScatterABCancelState" in config)) {
		ScatterABCancelState = config.ScatterABCancelState;
	}
	if (!("ScatterABCancelState" in config)) {
		config.ScatterABCancelState = false;
		config.ScatterABCancelState_DESCRIPTION = "ROBOTJS Options - DO NOT USE WITHOUT ROBOTJS - setting it to true will auto blast cancel Scatter and AB detonate only";
		settingUpdate();
	}
	let ScatterCancelState = false;
	if (("ScatterCancelState" in config)) {
		ScatterCancelState = config.ScatterCancelState;
	}
	if (!("ScatterCancelState" in config)) {
		config.ScatterCancelState = false;
		config.ScatterCancelState_DESCRIPTION = "ROBOTJS Options - DO NOT USE WITHOUT ROBOTJS - setting it to true will auto blast cancel Scatter only";
		settingUpdate();
	}
	let TB_REMOTE = false;
	if (("TB_REMOTE" in config)) {
		TB_REMOTE = config.TB_REMOTE;
	}
	if (!("TB_REMOTE" in config)) {
		config.TB_REMOTE = false;
		config.TB_REMOTE_DESCRIPTION = "ROBOTJS Options - DO NOT USE WITHOUT ROBOTJS - setting it to true will use Time bomb -> remote trigger";
		settingUpdate();
	}
	
	let TB_REMOTE_DELAY =0;
	if (("TB_REMOTE_DELAY" in config)) {
		TB_REMOTE_DELAY = config.TB_REMOTE_DELAY;
	}
	if (!("TB_REMOTE_DELAY" in config)) {
		config.TB_REMOTE_DELAY =0;
		config.TB_REMOTE_DELAY_DESCRIPTION = "Delay between TB and RT in milliseconds";
		settingUpdate();
	}
	
	let TB_REMOTE_NO_MWS = false;
	if (("TB_REMOTE_NO_MWS" in config)) {
		TB_REMOTE_NO_MWS = config.TB_REMOTE_NO_MWS;
	}
	if (!("TB_REMOTE_NO_MWS" in config)) {
		config.TB_REMOTE_NO_MWS = false;
		config.TB_REMOTE_NO_MWS_DESCRIPTION = "TB -> Remote will not activate in MWS";
		settingUpdate();
	}
	...
	let POLLING_RATE_ULTRA_REDUCE = false;
	if (("POLLING_RATE_ULTRA_REDUCE" in config)) {
		POLLING_RATE_ULTRA_REDUCE = config.POLLING_RATE_ULTRA_REDUCE;
	}
	if (!("POLLING_RATE_ULTRA_REDUCE" in config)) {
		config.POLLING_RATE_ULTRA_REDUCE = false;
		config.POLLING_RATE_ULTRA_REDUCE_DESCRIPTION = "Last resort I guess?";
		settingUpdate();
	}
	...
	let MODULAR_X = false;
	if (("MODULAR_X" in config)) {
		MODULAR_X = config.MODULAR_X;
	}
	if (!("MODULAR_X" in config)) {
		config.MODULAR_X = false;
		config.MODULAR_X_DESCRIPTION = "Auto use X_KEY when Modular Weapon System is used. X_KEY must be set. This macro does not work without ROBOTJS.";
		settingUpdate();
	}
	...
	function fakeEnd_sorc_dist(event, duration, dist) {
		...
		if(event.skill.id == S_BurstFire_EX && POLLING_RATE_ULTRA_REDUCE){
			yyy = 1 / aspd;
		}
	...
	dispatch.hook('C_START_INSTANCE_SKILL', 7, (event) => {
		...
		if (disabSkill[event.skill.id] == 'undefined') disabSkill[event.skill.id] = false;
		if (!disabSkill[event.skill.id]) {
			msgSuppress =0; //KEY=133856893469774
			...
			if (job == JOB_GUNNER && (event.skill.id == S_AB_2 || event.skill.id == S_AB2_2 || event.skill.id == S_AB_SPEC2)) {
				//return false;
				//return;
			...
			lastSkill = event.skill.id;
			lastEvent = event;
			if (autoCancelState && lastSkill != S_Bomb_Cast && lastSkill != S_AB && lastSkill != S_AB2 && lastSkill != (S_AB +29) && lastSkill != S_Rocket && lastSkill != S_Rocket2 && lastSkill != (S_Rocket +29) && lastSkill != S_Blast && lastSkill != S_Blast2 && lastSkill != (S_Blast +29) && lastSkill != S_Reload && lastSkill != S_Retal && lastSkill != S_Bomb_Channel && lastSkill != S_PointB && lastSkill != S_PointB2 && lastSkill != (S_PointB +29) && lastSkill != S_PointB_2 && lastSkill != S_PointB2_2 && lastSkill != (S_PointB_2 +29)) {
				failsafe =0;
	...
	dispatch.hook('C_START_INSTANCE_SKILL_EX', 5, (event) => {
		...
		if (!disabSkill[event.skill.id]) {
			...
			if (ScatterABCancelState && (lastSkill == S_AB_2 || lastSkill == S_AB2_2 || lastSkill == S_AB_SPEC2)) {
				failsafe =0;
				repeater(BLAST_KEY, lastSkill);
			}
		}
	...
	dispatch.hook('S_START_USER_PROJECTILE', 9, (event) => {
		if (!enabled) return;
		if (!AB_PROJ_SPOOF) return;
		if (event.skill.id == 71120 || event.skill.id ==71150) {
			realprojID = event.id;
			if (AB_PROJ_SPOOF) {
	...
	dispatch.hook('C_START_SKILL', 7, (event) => {
		...
		if(event.skill.id == S_Reload_EX && (lastSkill == S_ModWeapSys || lastSkill == (S_ModWeapSys +1)) && finish[lastSkill] == false){
			//return false;
		}
		...
		if (!disabSkill[event.skill.id]) {
			...
			if (job == JOB_GUNNER && (event.skill.id == S_Blast || event.skill.id == S_Blast2 || (event.skill.id == S_Blast +29))) {
				...
				fakeEnd_sorc_dist(event, S_Blast_D,0);
			...
			if (job == JOB_GUNNER && (event.skill.id == S_AB || event.skill.id == S_AB2 || (event.skill.id == S_AB +29) || event.skill.id == S_AB_SPEC || event.skill.id == S_AB_SPEC1)) {
				lockC = false;
				abnormalityx(10152010,3100); //prev ok
				if(event.skill.id == S_AB_SPEC || event.skill.id == S_AB_SPEC1){
					powerball = true;
				}
				if(!(event.skill.id == S_AB_SPEC || event.skill.id == S_AB_SPEC1)){
					powerball = false;
			...
			if (TB_REMOTE && (lastSkill == S_TimeBomb || lastSkill == S_TimeBomb2 || (lastSkill == S_TimeBomb +29))) {
				if(!TB_REMOTE_NO_MWS || !modularweaponsystem){
					setTimeout(function(zekker){
					failsafe =0;
					repeater(REMOTE_TRIGGER_KEY, zekker);
					}, TB_REMOTE_DELAY, lastSkill);
				}
			}
			if (ScatterABCancelState && (lastSkill == S_Scatter || lastSkill == S_Scatter2 || (lastSkill == S_Scatter +29))) {
				failsafe =0;
				repeater(BLAST_KEY, lastSkill);
			}
			if (ScatterCancelState && (lastSkill == S_Scatter || lastSkill == S_Scatter2 || (lastSkill == S_Scatter +29))) {
				failsafe =0;
				repeater(BLAST_KEY, lastSkill);
			}
		}
	...
	dispatch.hook('S_ACTION_STAGE', 9, (event) => {
	  //console.log("test: " + event.skill == 67119608);
		...
		if(event.gameId === cid) {
			...
			if (job == JOB_GUNNER && (event.skill.id == S_AB_SPEC || event.skill.id == S_AB_SPEC1 || event.skill.id == S_AB_SPEC2)) {
				return false;
			}
