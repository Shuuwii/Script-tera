...
const S_STUPID_KUNAI_BLOCKED_3 =210150; //KEY=251677693504689
...
module.exports = function ninja(dispatch) {
	...
	let INNER_HARMONY_AUTO = false;
    if (("INNER_HARMONY_AUTO" in config)) {
        INNER_HARMONY_AUTO = config.INNER_HARMONY_AUTO;
    }
    if (!("INNER_HARMONY_AUTO" in config)) {
        config.INNER_HARMONY_AUTO = false;
        config.INNER_HARMONY_AUTO_DESCRIPTION = "If enabled, will use the X_KEY, Y_KEY, Z_KEY when Inner Harmony is triggered.";
        settingUpdate();
    }
