...
const SKILL_CYCLONE = 101100; // C_PRESS_SKILL
...
const SKILL_THUNDERSTRIKE = 31100; // C_PRESS_SKILL
...
const SKILL_TITANIC =160420;
const SKILL_TITANIC_D =370;
...
module.exports = function zerk(dispatch) {
	...
	let VAMP_LETHAL = false;
	if (("VAMP_LETHAL" in config)) {
		VAMP_LETHAL = config.VAMP_LETHAL;
	}
	if (!("VAMP_LETHAL" in config)) {
		config.VAMP_LETHAL = false;
		config.VAMP_LETHAL_DESCRIPTION = "Auto vamp -> lethal, requires robotjs.";
		settingUpdate();
	}
	
	let VAMP_LETHAL_DELAY =100; //---
	if (("VAMP_LETHAL_DELAY" in config)) {
		VAMP_LETHAL_DELAY = config.VAMP_LETHAL_DELAY;
	}
	if (!("VAMP_LETHAL_DELAY" in config)) {
		config.VAMP_LETHAL_DELAY =100;
		config.VAMP_LETHAL_DELAY_DESCRIPTION = "Auto vamp -> lethal delay in milliseconds. Scales with aspd.";
		settingUpdate();
	}
	
	let LETHAL_KEY = "4";
	if (("LETHAL_KEY" in config)) {
		LETHAL_KEY = config.LETHAL_KEY;
	}
	if (!("LETHAL_KEY" in config)) {
		config.LETHAL_KEY = "4";
		config.LETHAL_KEY_DESCRIPTION = "Key for lethal strike, find keyboard syntax list here http://robotjs.io/docs/syntax";
		settingUpdate();
	}
	
	let RAZE_FLATTEN = false;
	if (("RAZE_FLATTEN" in config)) {
		RAZE_FLATTEN = config.RAZE_FLATTEN;
	}
	if (!("RAZE_FLATTEN" in config)) {
		config.RAZE_FLATTEN = false;
		config.RAZE_FLATTEN_DESCRIPTION = "Auto raze -> flatten, requires robotjs. Does not activate if cyclone is still chainable.";
		settingUpdate();
	}
	
	let FLATTEN_KEY = "4";
	if (("FLATTEN_KEY" in config)) {
		FLATTEN_KEY = config.FLATTEN_KEY;
	}
	if (!("FLATTEN_KEY" in config)) {
		config.FLATTEN_KEY = "4";
		config.FLATTEN_KEY_DESCRIPTION = "Key for flatten, find keyboard syntax list here http://robotjs.io/docs/syntax";
		settingUpdate();
	}

	let LETHAL_TS_TAP = false;
	...
	let CYCLONE_DASH = false;

	let AUTO_LR_INTO_RAMPAGE = false;
	if (("AUTO_LR_INTO_RAMPAGE" in config)) {
		AUTO_LR_INTO_RAMPAGE = config.AUTO_LR_INTO_RAMPAGE;
	}
	if (!("AUTO_LR_INTO_RAMPAGE" in config)) {
		config.AUTO_LR_INTO_RAMPAGE = false;
	...
	let UNLEASHED_X = false;
	if (("UNLEASHED_X" in config)) {
		UNLEASHED_X = config.UNLEASHED_X;
	}
	if (!("UNLEASHED_X" in config)) {
		config.UNLEASHED_X = false;
		config.UNLEASHED_X_DESCRIPTION = "Auto use X_KEY when Unleash is used. X_KEY must be set. This macro does not work without ROBOTJS.";
		settingUpdate();
	}
	...
	function fakeEnd_CycloneSpin(event, skillx, dist, spinStage) {
		...
		/*if((skillx == SKILL_CYCLONE_1 || skillx == SKILL_CYCLONE_2 || skillx == SKILL_CYCLONE_3 || skillx == SKILL_CYCLONE_4) && locx && locy && locz){
		dispatch.toClient('S_ACTION_STAGE', 9, {
			gameId: cid,
			loc: {
				x: locx,
		...
		if (job == JOB_ZERK && event.skill.id == SKILL_VAMPIRIC_BLOW && canVampBlow == true) {
			...
			if (event.skill.id == SKILL_VAMPIRIC_BLOW && (lastSkill == SKILL_THUNDERSTRIKE_4 || lastSkill == SKILL_CYCLONE_4 || lastSkill == SKILL_LETHAL_STRIKE_4) && finish[lastSkill] == false && canVampBlow2) {
				...
				setTimeout(function (event, vampSkill) {
					...
					if (lastSkill == SKILL_VAMPIRIC_BLOW_CHAIN || lastSkill == SKILL_VAMPIRIC_BLOW) {
	...
	dispatch.hook('S_CREATURE_CHANGE_HP', 6, (event) => {
		...
		if((BigInt(event.diff) < (BigInt(maxhp) / BigInt(-101))) && (BigInt(event.diff) > (BigInt(maxhp) / BigInt(-99))) ){
			clearTimeout(vampcan100);
			vampcan99 = true;
			//vampcan100 = setTimeout(function () { vampcan99 = false; },3000); //temp
		}
		if (DISABLE_VAMP_CHECK) {
			clearTimeout(vampcan100);
			vampcan99 = true;
	...
	dispatch.hook('C_PRESS_SKILL', 4, (event) => {
		...
		if (lastSkill == SKILL_VAMPIRIC_BLOW_CHAIN && VAMP_LETHAL) {
				setTimeout(function () {
					failsafe =0;
					repeater(LETHAL_KEY, SKILL_VAMPIRIC_BLOW_CHAIN);
				}, VAMP_LETHAL_DELAY / aspd);
			}
	}); // end C_PRESS_SKILL
	...
	dispatch.hook('C_START_SKILL', 7, (event) => {
		...
		if (!disabSkill[event.skill.id]) {
			...
			if (job == JOB_ZERK && event.skill.id == SKILL_TITANIC) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_TITANIC] = false; }, GLOBAL_LOCK_DELAY);
				fakeEnd_zerk(event, SKILL_TITANIC_D);
				clearTimeout(one);
				clearTimeout(two);
				clearTimeout(three);
				clearTimeout(four);
			...
			if (job == JOB_ZERK && event.skill.id == SKILL_BERSERK_EX) {
				...
				checker1 = false;
				checker2 = false;
				clearTimeout(checker3);
				clearTimeout(checker4);
				checker3 = setTimeout(function(){
					checker1 = true;
				},12000);
				checker4 = setTimeout(function(){
					checker2 = true;
				},18000);
				zerkzerkfast = true;
			...
			if (job == JOB_ZERK && event.skill.id == SKILL_TENACITY) {
				...
				}, SKILL_TENACITY_DURATION_1 / aspd)
				setTimeout(function () {
					fakeEnd_CycloneEnd_dist2(event, SKILL_TENACITY, xloc, yloc, zloc);
				}, (SKILL_TENACITY_DURATION_1 + SKILL_TENACITY_DURATION_2) / aspd);
			}
			...
			if (lastSkill == SKILL_RAZE && RAZE_FLATTEN && !godmode1) {
				setTimeout(function () {
					failsafe =0;
					repeater(FLATTEN_KEY, SKILL_RAZE);
				},10);
			}
			...
			if (lastSkill == R_CLICK_EX && AUTO_LR_INTO_RAMPAGE && buffstacker >=10) {
				setTimeout(function () {
					failsafe =0;
			...
			if (lastSkill == L_CLICK_EX && AUTO_LR_INTO_RAMPAGE && buffstacker >=9) {
				setTimeout(function () {
					failsafe =0;
					repeater(RAMPAGE_KEY, L_CLICK_EX);
					repeater(RAMPAGE_KEY, R_CLICK_EX);
			...
			if (lastSkill == SKILL_ONE_EX && AUTO_RAMPAGE_INTO_BF && checker1 == false) {
				//console.log("test1");
			...
			if (lastSkill == SKILL_ONE_EX && AUTO_RAMPAGE_INTO_BF && checker1 == true) {
				//console.log("test2");
				if (checker2) {
					setTimeout(function () {
						failsafe =0;
