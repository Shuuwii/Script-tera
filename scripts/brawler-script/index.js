...
const SKILL_RETALIATE_DURATION =1000; //KEY=263980042282641
...
const SKILL_FLY_KICK_DURATION = 1810; //1205
...
const SKILL_ONE_INCH_DURATION = 2000; //955 20%
...
module.exports = function brawler(dispatch) {
	...
	let AUTO_BLOCK_FLIP_KICK = false;
	if (("AUTO_BLOCK_FLIP_KICK" in config)) {
		AUTO_BLOCK_FLIP_KICK = config.AUTO_BLOCK_FLIP_KICK;
	}
	if (!("AUTO_BLOCK_FLIP_KICK" in config)) {
		config.AUTO_BLOCK_FLIP_KICK = false;
		config.AUTO_BLOCK_FLIP_KICK_DESCRIPTION = "DO NOT USE WITHOUT ROBOTJS - Auto block cancel flip kick";
		settingUpdate();
	}
	let AUTO_BLOCK_FLIP_KICK_DELAY =1000;
	if (("AUTO_BLOCK_FLIP_KICK_DELAY" in config)) {
		AUTO_BLOCK_FLIP_KICK_DELAY = config.AUTO_BLOCK_FLIP_KICK_DELAY;
	}
	if (!("AUTO_BLOCK_FLIP_KICK_DELAY" in config)) {
		config.AUTO_BLOCK_FLIP_KICK_DELAY =1000;
		config.AUTO_BLOCK_FLIP_KICK_DELAY_DESCRIPTION = "Delay at base aspd in milliseconds";
		settingUpdate();
	}
	
	let JH_AUTO_HM = false;
	if (("JH_AUTO_HM" in config)) {
		JH_AUTO_HM = config.JH_AUTO_HM;
	}
	if (!("JH_AUTO_HM" in config)) {
		config.JH_AUTO_HM = false;
		config.JH_AUTO_HM_DESCRIPTION = "DO NOT USE WITHOUT ROBOTJS - Auto Haymaker after Jackhammer.";
		settingUpdate();
	}
	let JH_AUTO_HM_DELAY =1000;
	if (("JH_AUTO_HM_DELAY" in config)) {
		JH_AUTO_HM_DELAY = config.JH_AUTO_HM_DELAY;
	}
	if (!("JH_AUTO_HM_DELAY" in config)) {
		config.JH_AUTO_HM_DELAY =1000;
		config.JH_AUTO_HM_DELAY_DESCRIPTION = "Delay at base aspd in milliseconds";
		settingUpdate();
	}
	let HM_KEY = "f1";
	if (("HM_KEY" in config)) {
		HM_KEY = config.HM_KEY;
	}
	if (!("HM_KEY" in config)) {
		config.HM_KEY = "f1";
		config.HM_KEY_DESCRIPTION = "Key for Haymaker, find keyboard syntax list here http://robotjs.io/docs/syntax";
		settingUpdate();
	}
	
	let FLYKICK_AUTO_ONEINCH = false;
	if (("FLYKICK_AUTO_ONEINCH" in config)) {
		FLYKICK_AUTO_ONEINCH = config.FLYKICK_AUTO_ONEINCH;
	}
	if (!("FLYKICK_AUTO_ONEINCH" in config)) {
		config.FLYKICK_AUTO_ONEINCH = false;
		config.FLYKICK_AUTO_ONEINCH_DESCRIPTION = "DO NOT USE WITHOUT ROBOTJS - Auto One Inch after Fly Kick.";
		settingUpdate();
	}
	let FLYKICK_AUTO_ONEINCH_DELAY =100;
	if (("FLYKICK_AUTO_ONEINCH_DELAY" in config)) {
		FLYKICK_AUTO_ONEINCH_DELAY = config.FLYKICK_AUTO_ONEINCH_DELAY;
	}
	if (!("FLYKICK_AUTO_ONEINCH_DELAY" in config)) {
		config.FLYKICK_AUTO_ONEINCH_DELAY =100;
		config.FLYKICK_AUTO_ONEINCH_DELAY_DESCRIPTION = "Delay at base aspd in milliseconds";
		settingUpdate();
	}
	let ONEINCH_KEY = "f2";
	if (("ONEINCH_KEY" in config)) {
		ONEINCH_KEY = config.ONEINCH_KEY;
	}
	if (!("ONEINCH_KEY" in config)) {
		config.ONEINCH_KEY = "f2";
		config.ONEINCH_KEY_DESCRIPTION = "Key for One Inch, find keyboard syntax list here http://robotjs.io/docs/syntax";
		settingUpdate();
	}
	
	let PD_AUTO_CP = false;
	if (("PD_AUTO_CP" in config)) {
		PD_AUTO_CP = config.PD_AUTO_CP;
	}
	if (!("PD_AUTO_CP" in config)) {
		config.PD_AUTO_CP = false;
		config.PD_AUTO_CP_DESCRIPTION = "DO NOT USE WITHOUT ROBOTJS - Auto Counter Punch after Piledriver.";
		settingUpdate();
	}
	let PD_AUTO_CP_DELAY =1000;
	if (("PD_AUTO_CP_DELAY" in config)) {
		PD_AUTO_CP_DELAY = config.PD_AUTO_CP_DELAY;
	}
	if (!("PD_AUTO_CP_DELAY" in config)) {
		config.PD_AUTO_CP_DELAY =1000;
		config.PD_AUTO_CP_DELAY_DESCRIPTION = "Delay at base aspd in milliseconds";
		settingUpdate();
	}
	let PD_AUTO_RHK = false;
	if (("PD_AUTO_RHK" in config)) {
		PD_AUTO_RHK = config.PD_AUTO_RHK;
	}
	if (!("PD_AUTO_RHK" in config)) {
		config.PD_AUTO_RHK = false;
		config.PD_AUTO_RHK_DESCRIPTION = "DO NOT USE WITHOUT ROBOTJS - Auto RHK after Piledriver. Cannot be used with PD_AUTO_CP";
		settingUpdate();
	}
	let HM_AUTO_CP = false;
	if (("HM_AUTO_CP" in config)) {
		HM_AUTO_CP = config.HM_AUTO_CP;
	}
	if (!("HM_AUTO_CP" in config)) {
		config.HM_AUTO_CP = false;
		config.HM_AUTO_CP_DESCRIPTION = "DO NOT USE WITHOUT ROBOTJS - Auto Counter Punch after Haymaker";
		settingUpdate();
	}
	let CP_KEY = "f3";
	if (("CP_KEY" in config)) {
		CP_KEY = config.CP_KEY;
	}
	if (!("CP_KEY" in config)) {
		config.CP_KEY = "f3";
		config.CP_KEY_DESCRIPTION = "Key for Counter Punch, find keyboard syntax list here http://robotjs.io/docs/syntax";
		settingUpdate();
	}
	
	let JH_AUTO_PD = false;
	if (("JH_AUTO_PD" in config)) {
		JH_AUTO_PD = config.JH_AUTO_PD;
	}
	if (!("JH_AUTO_PD" in config)) {
		config.JH_AUTO_PD = false;
		config.JH_AUTO_PD_DESCRIPTION = "DO NOT USE WITHOUT ROBOTJS - Auto piledriver after jackhammer";
		settingUpdate();
	}
	let PD_KEY = "f3";
	if (("PD_KEY" in config)) {
		PD_KEY = config.PD_KEY;
	}
	if (!("PD_KEY" in config)) {
		config.PD_KEY = "f3";
		config.PD_KEY_DESCRIPTION = "Key for Pile Driver, find keyboard syntax list here http://robotjs.io/docs/syntax";
		settingUpdate();
	}
	let JH_AUTO_PD_DELAY =1000;
	if (("JH_AUTO_PD_DELAY" in config)) {
		JH_AUTO_PD_DELAY = config.JH_AUTO_PD_DELAY;
	}
	if (!("JH_AUTO_PD_DELAY" in config)) {
		config.JH_AUTO_PD_DELAY =1000;
		config.JH_AUTO_PD_DELAY_DESCRIPTION = "Delay at base aspd in milliseconds";
		settingUpdate();
	}
	...
	if (!("GF_X" in config)) {
		config.GF_X = false;
		config.GF_X_DESCRIPTION = "Auto use X_KEY when Growing Fury is used. X_KEY must be set. This macro does not work without ROBOTJS.";
		settingUpdate();
	}
	...
	function fakeEnd_sorc(event, duration) {
		...
		dispatch.toClient('S_ACTION_STAGE', 9, {
			...
			stage: 0,
			speed: aspd * aaa,
			...(dispatch.majorPatchVersion >= 75 ? { projectileSpeed: aspd * aaa } : 0n),
		...
		timer[event.skill.id] = setTimeout(
			function (event) {
				...
				if (lastSkill == SKILL_RAMPAGE) { return; }
				if (event.skill.id != lastSkill) { return false; }
	...
	function fakeEnd_sorc_dist(event, duration, dist) {
		...
		dispatch.toClient('S_ACTION_STAGE', 9, {
			...
			stage: 0,
			speed: skillSpeed,
			...(dispatch.majorPatchVersion >= 75 ? { projectileSpeed: skillSpeed } : 0n),
			id: atkid[event.skill.id],
			effectScale: 1.0, moving: false, dest: { x: 0, y: 0, Z: 0 }, target: 0n, animSeq: [],
		});
		}
		...
		timer[event.skill.id] = setTimeout(
			function (event) {
				skillActive = false;
				if (event.skill.id != lastSkill && event.skill.id != SKILL_GROWING_FURY && event.skill.id != (SKILL_GROWING_FURY +1) && event.skill.id != (SKILL_GROWING_FURY +29)) { return; }
				if (!lastFury && (event.skill.id == SKILL_GROWING_FURY || event.skill.id == (SKILL_GROWING_FURY +1) || event.skill.id == (SKILL_GROWING_FURY +29))) { return; }
				if (lastSkill == SKILL_RAMPAGE) { return; }
				if (lastSkill == SKILL_BULLRUSH) { return false; }
				dispatch.toClient('S_ACTION_END', 5, {
					gameId: cid,
	...
	dispatch.hook('S_ABNORMALITY_BEGIN', 4, (event) => {
		...
		if (event.id ==10153190) {
			...
			soundcounter2 = setTimeout(function () { soundcounter = 0; },4000);
		}
	...
	dispatch.hook('C_PRESS_SKILL', 4, (event) => {
		...
		if (job == JOB_BRAWLER && event.skill.id == SKILL_COUNTER && event.press == false) {
			if (lastSkill == SKILL_COUNTER) { skillActive = false; }
		...
		if (job == JOB_BRAWLER && event.skill.id == SKILL_BULLRUSH && event.press == true) {
			...
			dispatch.toClient('S_ACTION_STAGE', 9, {
				...
				animSeq: [{ duration: 2950, xyRate: 1, zRate: 1, distance: 340, }],
			});
	...
	dispatch.hook('S_START_COOLTIME_SKILL', 3, (event) => {
		...
		if (event.skill.id == SKILL_HAYMAKER) {
			...
			if(event.cooldown >= (SKILL_HAYMAKER_DURATION / aspd)){
				...
				HMDis = setTimeout(function () { disabSkill[SKILL_HAYMAKER] = false; }, event.cooldown);
			}
		}
	});
	...
	dispatch.hook('C_START_INSTANCE_SKILL', 7, (event) => {
		...
		if (job == JOB_BRAWLER && event.skill.id == (SKILL_DIVINE_WRATH +1)) {
				disabSkill[event.skill.id] = true;
				setTimeout(function () { disabSkill[(SKILL_DIVINE_WRATH +1)] = false; }, GLOBAL_LOCK_DELAY / aspd);
				...
				dispatch.toClient('S_ACTION_STAGE', 9, {
						...
						...(dispatch.majorPatchVersion >= 75 ? { projectileSpeed: 1 } : 0n),
						id: atkid[event.skill.id],
						effectScale: 1.0, moving: false, dest: { x: event.loc.x, y: event.loc.y, z: event.loc.z }, target: 0n, animSeq: [],
					});
				setTimeout(function (event) {
					...
					dispatch.toClient('S_ACTION_STAGE', 9, {
						...
						...(dispatch.majorPatchVersion >= 75 ? { projectileSpeed: 1 } : 0n),
						id: atkid[event.skill.id],
						effectScale: 1.0, moving: false, dest: { x: event.endpoints[0].x, y: event.endpoints[0].y, z: event.endpoints[0].z }, target: 0n, animSeq: [],
					});
		...
		if (!disabSkill[event.skill.id] /*&& bullrushActive == false */) {
			...
			if (job == JOB_BRAWLER && (event.skill.id == SKILL_HAYMAKER || event.skill.id == SKILL_HAYMAKER_3 || event.skill.id == SKILL_HAYMAKER_2 || event.skill.id == SKILL_HAYMAKER_4 || event.skill.id == SKILL_HAYMAKER_5 || event.skill.id == SKILL_HAYMAKER_6 || event.skill.id == SKILL_HAYMAKER_7)) {
				...
				hayPart2Timer = setTimeout(function (event) {
					dispatch.toClient('S_ACTION_STAGE', 9, {
						gameId: cid,
						loc: { x: collisionLocX, y: collisionLocY, z: collisionLocZ },
						w: event.w,
						templateId: model,
						skill: event.skill.id,
						stage: 1,
				...
				if (HM_AUTO_CP) {
					setTimeout(function () {
						if ((lastSkill == SKILL_HAYMAKER || lastSkill == SKILL_HAYMAKER_3 || lastSkill == SKILL_HAYMAKER_2 || lastSkill == SKILL_HAYMAKER_4 || lastSkill == SKILL_HAYMAKER_5 || lastSkill == SKILL_HAYMAKER_6 || lastSkill == SKILL_HAYMAKER_7) && counterAvailable) {
							var robot97 = require("robotjs");
							robot97.keyTap(CP_KEY);
						}
					}, HMRHKDelay / aspd);
				}
			}
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_GROUNDPOUND) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_GROUNDPOUND] = false; }, GLOBAL_LOCK_DELAY * 0.7 / aspd);
				if (skillActive) force_end(lastEvent,4);
				skillActive = true;
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_TAUNT) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_TAUNT] = false; }, GLOBAL_LOCK_DELAY / aspd);
				if (skillActive) force_end(lastEvent,4);
				skillActive = true;
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_ENRAGE) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_ENRAGE] = false; }, GLOBAL_LOCK_DELAY / aspd);
				if (skillActive) force_end(lastEvent,4);
				skillActive = true;
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_FLY_KICK) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_FLY_KICK] = false; }, GLOBAL_LOCK_DELAY / aspd);
				if (skillActive) {
				...
				if (skillActive) force_end(lastEvent,4);
				skillActive = true;
				if (FLYKICK_AUTO_ONEINCH) {
					setTimeout(function () {
						if (lastSkill == SKILL_FLY_KICK) {
							failsafe =0;
							repeater(ONEINCH_KEY, SKILL_FLY_KICK);
						}
						if (lastSkill == (SKILL_FLY_KICK +30)) {
							failsafe =0;
							repeater(ONEINCH_KEY, (SKILL_FLY_KICK +30));
						}
					}, FLYKICK_AUTO_ONEINCH_DELAY / aspd);
				}
			}

			if (job == JOB_BRAWLER && event.skill.id == SKILL_ONE_INCH) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_ONE_INCH] = false; }, GLOBAL_LOCK_DELAY / aspd);
				if (speedipman) {
					event.skill.id = event.skill.id -1;
					if (skillActive) {
						event.skill.id = event.skill.id +30;
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_ONE_INCH_2) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_ONE_INCH_2] = false; }, GLOBAL_LOCK_DELAY / aspd);
				event.skill.id = event.skill.id -1;
				if (speedipman) {
					event.skill.id = event.skill.id -1;
					if (skillActive) {
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_SOUND) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_SOUND] = false; }, GLOBAL_LOCK_DELAY / 3 / aspd);
				...
				if (soundcounter ==0) {
					...
					soundlock = true;
					clearTimeout(soundlock2);
					soundlock2 = setTimeout(function () { soundlock = false; }, (SKILL_SOUND_DURATION + SKILL_SOUND_DURATION_2) / aspd);
				}
				if (soundcounter ==1) {
					...
					soundlock = true;
					clearTimeout(soundlock2);
					soundlock2 = setTimeout(function () { soundlock = false; }, SKILL_SOUND_DURATION_3 / aspd);
				}
				if (soundcounter ==2) {
				...
				if (soundcounter ==3) {
					...
					soundlock = true;
					clearTimeout(soundlock2);
					soundlock2 = setTimeout(function () { soundlock = false; }, SKILL_SOUND_DURATION_4 / aspd);
				}
				if (soundcounter ==4) {
					...
					soundlock = true;
					clearTimeout(soundlock2);
					soundlock2 = setTimeout(function () { soundlock = false; }, SKILL_SOUND_DURATION_5 / aspd);
				}
				if (soundcounter ==5) {
					...
					soundlock = true;
					clearTimeout(soundlock2);
					soundlock2 = setTimeout(function () { soundlock = false; }, SKILL_SOUND_DURATION_6 / aspd);
				}
				if (soundcounter ==6) {
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_RETALIATE) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_RETALIATE] = false; }, GLOBAL_LOCK_DELAY / aspd);
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_COUNTER_PUNCH && counterAvailable) {
				var timer = setTimeout(function () { disabSkill[SKILL_COUNTER_PUNCH] = false; }, GLOBAL_LOCK_DELAY / aspd);
				clearTimeout(counterTimer);
				if (skillActive) force_end(lastEvent,4);
				skillActive = true;
			...
			if (job == JOB_BRAWLER && (event.skill.id == SKILL_PILEDRIVER || event.skill.id == SKILL_PILEDRIVER_3 || event.skill.id == SKILL_PILEDRIVER_5)) {
				...
				if (disabSkill[SKILL_PILEDRIVER] != true) {
					disabSkill[event.skill.id] = true;
					var timer = setTimeout(function () { disabSkill[SKILL_PILEDRIVER] = false; }, GLOBAL_LOCK_DELAY / aspd);
					if (skillActive) force_end(lastEvent,4);
					skillActive = true;
				...
				if (PD_AUTO_CP) {
					setTimeout(function () {
						if ((lastSkill == SKILL_PILEDRIVER || lastSkill == SKILL_PILEDRIVER_3 || lastSkill == SKILL_PILEDRIVER_5) && counterAvailable) {
							var robot97 = require("robotjs");
							robot97.keyTap(CP_KEY);
						}
					}, PD_AUTO_CP_DELAY / aspd);
				}
				if (PD_AUTO_RHK) {
					setTimeout(function () {
						if ((lastSkill == SKILL_PILEDRIVER || lastSkill == SKILL_PILEDRIVER_3 || lastSkill == SKILL_PILEDRIVER_5)) {
							var robot97 = require("robotjs");
							robot97.keyTap(RHKKEY);
						}
					}, PD_AUTO_CP_DELAY / aspd);
				}
			}
			...
			if (job == JOB_BRAWLER && (event.skill.id == SKILL_JACKHAMMER || event.skill.id == SKILL_JACKHAMMER_3 || event.skill.id == SKILL_JACKHAMMER_5)) {
				...
				if (JH_AUTO_HM) {
					setTimeout(function () {
						if (lastSkill == SKILL_JACKHAMMER || lastSkill == SKILL_JACKHAMMER_3 || lastSkill == SKILL_JACKHAMMER_5) {
							var robot97 = require("robotjs");
							robot97.keyTap(HM_KEY);
						}
					}, JH_AUTO_HM_DELAY);
				}
				if (JH_AUTO_PD) {
			...
			if (job == JOB_BRAWLER && (event.skill.id == SKILL_FLIPKICK || event.skill.id == SKILL_FLIPKICK_3)) {
				...
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_FLIPKICK] = false; }, GLOBAL_LOCK_DELAY / aspd);
				if (skillActive) force_end(lastEvent,4);
				skillActive = true;
				fakeEnd_sorc(event, SKILL_FLIPKICK_DURATION);
				if (AUTO_BLOCK_FLIP_KICK) {
					setTimeout(function () {
						if (lastSkill == SKILL_FLIPKICK || lastSkill == SKILL_FLIPKICK_3) {
							var robot95 = require("robotjs");
							robot95.keyTap(BLOCKKEY);
						}
					}, AUTO_BLOCK_FLIP_KICK_DELAY / aspd);
				}
			}
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_PUNCH2) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_PUNCH2] = false; }, GLOBAL_LOCK_DELAY / aspd);
				skillActive = true;
				yolo1();
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_PUNCH3) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_PUNCH3] = false; }, GLOBAL_LOCK_DELAY / aspd);
				skillActive = true;
				yolo1();
			...
			if (job == JOB_BRAWLER && event.skill.id == SKILL_PUNCH4) {
				disabSkill[event.skill.id] = true;
				var timer = setTimeout(function () { disabSkill[SKILL_PUNCH4] = false; }, GLOBAL_LOCK_DELAY / aspd);
				skillActive = true;
				yolo1();
			...
			if (job == JOB_BRAWLER && (event.skill.id == SKILL_PUNCH_CHAIN_START || event.skill.id == SKILL_PUNCH_CHAIN_START_2 || event.skill.id == SKILL_PUNCH_CHAIN_START_3)) {
				//event.skill.id = SKILL_PUNCH_CHAIN_START;
				//disabSkill[event.skill.id] = true;
				//var timer = setTimeout(function(){disabSkill[SKILL_PUNCH_CHAIN_START] = false;},0);
			...
			if (job == JOB_BRAWLER && (event.skill.id == SKILL_GROWING_FURY || event.skill.id == (SKILL_GROWING_FURY +1) || event.skill.id == (SKILL_GROWING_FURY +29)) && myRE ==3000) {
				disabSkill[event.skill.id] = true;
				setTimeout(function (event) { disabSkill[event.skill.id] = false; }, GLOBAL_LOCK_DELAY / aspd, event);
				if (skillActive) force_end(lastEvent,4);
				skillActive = true;
			...
			if (job == JOB_BRAWLER && (event.skill.id == SKILL_QUICK_DASH || event.skill.id == (SKILL_QUICK_DASH +30))) {
				...
				var timer = setTimeout(function () { disabSkill[SKILL_QUICK_DASH] = false; }, GLOBAL_LOCK_DELAY / 2 / aspd);
				if (skillActive) force_end(lastEvent,6);
				skillActive = true;
